...
#assert async dummy nil nil (ff :: nil) (sto ff).
% ttR
#assert syncR dummy nil nil tt.
% decideR -> ttR
#assert async dummy nil nil nil (sto tt).
% ttL -> decideR -> ttR
#assert async dummy nil nil (tt :: nil) (sto tt).
% andL -> ffL (and-RHS doesn't do anything)
#assert async dummy nil nil ((and ff ff) :: nil) (sto ff).
% andL -> ttL -> ffL
#assert async dummy nil nil ((and tt ff) :: nil) (sto ff).
% orL -> ffL | ffL
#assert async dummy nil nil ((or ff ff) :: nil) (sto ff).
% storeR -> decideR -> ttR
...
% andR -> ttR | ttR
#assert syncR dummy nil nil (and tt tt).
% orR -> ttR (left)
#assert syncR dummy nil nil (or tt ff).
% orR -> ttR (right)
#assert syncR dummy nil nil (or ff tt).
% releaseL -> ffL
#assert syncL dummy nil nil ff (sto ff).
% impL -> (releaseL -> ffL) | ttR
#assert syncL dummy nil nil (imp tt ff) (sto ff).


% releaseR -> impR -> ffL (resolving as fast as we can)
#assert syncR dummy nil nil (imp ff ff).
